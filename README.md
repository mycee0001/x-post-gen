...
lean-canvas.md を唯一の真実の源 (SoT) として、キャンバスに沿った **ポスト 5 候補 / 引用ツイート 5 候補 / リアルタイムリプライ 5 候補** を 1 コマンドで自動生成します。
...
| コマンド | 用途 | 生成数 | 履歴 |
|---|---|---|---|
| `/x-post` | 新規ポスト原稿を 5 つの異なる切り口で生成 | 5 | あり |
| `/x-quote` | X 上の関連ツイートを検索し、Top 5 に対するコメント原稿を生成 | 5 | あり |
| `/x-reply` | **リアルタイム**(直近6〜12h)の関連ポストを取得し、Top 5 にリプライ原稿を生成 | 5 | なし |
...
/x-post     # 新規ポスト 5 候補
/x-quote    # 引用ツイート 5 候補
/x-reply    # リアルタイムリプライ 5 候補(直近6〜12時間のポスト対象)
```
...
### `/x-reply` — リアルタイムリプライ生成

```
あなた: /x-reply
```

**リアルタイム性重視のコマンド**。引用ツイートと違い、会話参加が目的。

1. `lean-canvas.md` 読み込み
2. TwitterAPI.io で **直近 12 時間、min_likes 2、最新順** で関連ツイートを取得(3 クエリ × 20 件)
3. 関連性 × 新鮮度 × 会話参加性でスコアリング
   - エンゲージメント(いいね/RT)の重みは小さめ。「まだバズっていないがトピックに合うポスト」を優先
4. Top 5 を選定
5. 各候補にリプライ原稿を生成(対話的なトーン、200字以内、絵文字 0〜1個、ハッシュタグなし)
6. 炎上チェックを実施
7. リプライ元 URL + リプライ原稿を 5 件表示
8. **履歴追記なし**(リプライの特性上、後日同じポストに返しても自然)

💡 **リアルタイム性が命** のコマンドなので、生成後は早めに投稿することを推奨します。
...
| API | 想定利用 | コスト/月 |
|---|---|---|
| TwitterAPI.io | `/x-quote` 月 6 回 + `/x-reply` 月 10 回 × 検索 3 クエリ × 20〜30 件 ≒ 900 ツイート | 約 $0.14 |
| Tavily (Advanced search) | `/x-post` 月 6 回 + `/x-quote` 月 6 回 = 12 リクエスト(`/x-reply` は原則スキップ) | **無料枠内 $0** |
| **合計** | | **約 $0.14 / 月** |

※ Tavily は月 1,000 リクエストまで無料枠(2026-04 時点)。
`/x-reply` はリアルタイム性重視のため Tavily 呼び出しを原則スキップします。
...
│   ├── x-reply/SKILL.md            # /x-reply の挙動定義
│   └── _x-shared/
...
### Q. `/x-reply` で履歴が残らないのはなぜ?

リプライは「会話の一部」であり、後日同じポストに別の返信を返すのも自然だからです。
一方 `/x-post` や `/x-quote` は「発信コンテンツ」なので、重複や反復を避けるために履歴で管理します。

### Q. `/x-quote` と `/x-reply` の違いは?

| 観点 | /x-quote | /x-reply |
|---|---|---|
| 目的 | 引用ツイートで発信 | リアルタイム会話参加 |
| 検索期間 | 直近 72 時間 | **直近 6〜12 時間** |
| min_likes | 5 以上 | 2 以上(新鮮なポストも拾う) |
...
`/x-post` 1 回 ≒ 無料枠内($0)、`/x-quote` 1 回 ≒ $0.01〜$0.02、`/x-reply` 1 回 ≒ $0.01 を目安に試算してください。
