...
- **Web リサーチ** — Perplexity Sonar Pro で一次情報・ニュース・論文を調査し出典 URL を付与
...
- [Perplexity Sonar API](https://www.perplexity.ai/settings/api) アカウント (Web 調査で使用)
...
3. Perplexity Sonar で関連ニュース・論文を 1 回調査 (`--mfg-preset` でドメインフィルタ)
...
| Perplexity Sonar Pro | `/x-post` 月 6 回 + `/x-quote` 月 6 回 = 12 リクエスト | 約 $0.36 |
| **合計** | | **約 $0.5〜$1** |

※ 月 30 ポスト相当を 1 回 5 候補生成 × 6 実行で賄う前提。
...
### Q. Perplexity ではなく Claude Code の WebSearch を使えますか?

現状は Perplexity Sonar Pro 固定です。citations の構造化と `search_domain_filter` の制御が
本ツールの品質に直結するためです。将来的に切り替えオプションを追加する可能性はあります。
...
Perplexity Sonar Pro は約 $0.03/リクエスト、TwitterAPI.io は $0.15/1,000ツイートです。
`/x-post` 1 回 ≒ $0.03、`/x-quote` 1 回 ≒ $0.08 を目安に試算してください。
...
- X 公式 API での自動投稿
- スケジュール投稿
- Grok API / WebSearch への Perplexity 代替
