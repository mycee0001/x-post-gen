...
- **自己ポジショニング明示句は連発しない** — 「ADHD向けアプリを作っている側でも」「現場でも」「私の専門領域でも」等を **5 リプ全件に同じ形で入れる / 冒頭に置く / 同フレーズを使い回す** のは禁止。ただし **5 リプ中 1〜2 件で、文中・文末に控えめに 1 句** なら許容。基本は **言葉の精度・機序の指摘・論点の接続** で暗黙に専門性を示す
- **主語は相手の論点に据える** — 主役はリプライ元のポスト。発信者の自己紹介ではない
- **文体は丁寧体（です・ます）、一人称は「私」**
- **一行最大 50 字、理想 35 字** で改行を入れる
- **中学生でもわかる単語が基本**。ただし **1 リプにつき 1 個までは専門用語(実行機能・ワーキングメモリ・過集中など)を原語で使ってよい**(機序を平易語より精度高く伝えられる場合 / 文脈で意味が掴めるか短く補足する場合)。連発・全件展開は禁止
