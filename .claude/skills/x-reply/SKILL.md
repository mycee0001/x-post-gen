...
- **対象は大元のポスト(トップレベル)のみ**。他人のポストへのリプライ(ぶら下がりツイート)は混戦ツリーの中で埋もれ、Premium+ の返信ブースト効果も発信元のフォロワーに届きにくいため、検索段階で `-filter:replies` により除外する。引用ツイートは大元のポストに該当するため対象に含む
- **関連性 × 新鮮度 × リーチでスコアリング**。reply_count はスコアに使わない(賛同・反論・釈明・bot通知狙いの合計値で代理指標として弱く、200 件超で炎上を拾うリスクが副次指標としての価値を上回るため)。`min_replies=2` のフィルタで完全な独り言だけを除外し、過剰反応比・謝罪/釈明キーワードによる炎上ペナルティ -0.30 を別途適用する
- ターゲットは **24 時間以内に投稿され、自サービス文脈と高関連の発信**
...
| max_replies | フィルタなし | スコアには使わず、200 超 + 過剰反応比で炎上ペナルティ |
| スコアリング | 関連性 × エンゲージメント | **関連性 × 新鮮度 × リーチ(reply_count はスコア外)** |
| 履歴管理 | あり | あり(全期間の重複排除) |
...
**`--min-replies 2`** は完全な独り言を除外するための最低ラインフィルタ。reply 数自体はスコアリングに使わない(賛同・反論・釈明の合計値で代理指標として弱く、副次指標としてもノイズが大きい)。
**`--exclude-replies` を必ず付与**(大元のポストのみを対象にする。ぶら下がりリプライは反応率が低く、文脈もスレッドに依存するため除外)。サブカテゴリ検索 (Step 2.5) と緩和検索 (Step 2.7) でも同様に付与する。
...
| 項目 | 重み | 計算 |
|---|---:|---|
| **関連性** | **0.45** | そのサービスの canvas の topic との一致度。`origin == "subcategory"` は最後に × 0.7 |
| **新鮮度** | **0.30** | `1.0 - (hours_since_post / 24)` を 0〜1 にクリップ。投稿直後ほど高い |
...
> **reply_count はスコアに含めない**(賛同・反論・釈明・bot通知狙いの合計値で代理指標として弱く、副次指標として 0.10 重みを当てるとノイズと炎上拾いリスクのほうが上回るため)。完全な独り言除外は `--min-replies 2` のフィルタで担保し、200 超 + 過剰反応比 > 0.5 や謝罪/釈明キーワードでの炎上検出は下記ペナルティで処理する。表示には `💬 reply N` を残し、人間がフロー上で異常値に気づけるようにする。
...
```
--- 候補 1 [SAFE] 関連性 0.85 / 新鮮度 0.8 (5時間前) / リーチ 0.7 ---
```

炎上ペナルティが付いた候補は次のように表示:

```
--- 候補 X [SAFE / 炎上候補:過剰反応比] 関連性 0.7 / 新鮮度 0.6 / リーチ 0.5 / ペナルティ -0.30 ---
```

reply 数はスコアには使わないが、表示の `💬 reply N` は残し、人間がフロー上で異常値(突然 200+ になっている、等)に気づけるようにする。
...
- **対象は大元のポストのみ**。`--exclude-replies` で `-filter:replies` を付与し、ぶら下がりリプライは Step 2 / 2.5 / 2.7 すべてで除外する
- **リアルタイム性が反応率の最重要因子** — 24h 厳守。古い投稿に拾わない
- **reply_count はスコアに含めない**。`min_replies=2` のフィルタで完全な独り言除外、200+ かつ過剰反応比トリガーで炎上ペナルティ -0.30、というレイヤー分離を徹底する
