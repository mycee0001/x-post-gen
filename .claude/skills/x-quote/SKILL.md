...
- **完全同意スタンスの強制** — 引用元への否定・反論・相対化・上から目線の指摘・敵視メタファーは絶対禁止。許される姿勢は (1) 同方向の抽象化 / (2) 同方向の具体化 / (3) 同方向のパターン適用 / (4) 読み手向けの実践示唆 の 4 種類のみ(詳細は `japanese_writing_style.md` 第 9 節)
- **冒頭でカギカッコ引用パターンを使わない**（「『〇〇』、〜ですね。」のような書き出しは禁止）。元ポストの語句に触れるのは、自分の主張がその語句に明確に紐づくときのみ、自分の文に自然に織り込む
- **自己ポジショニング明示句を使わない**（「ADHD向けプロダクトを作っている側として」「当事者向け設計でも」「現場でも」等）。権威は表に出さず、自分の知識範囲を使って役立つ考え・示唆で呼応する形にする。専門性は **言葉の精度・機序の指摘・論点の接続** で暗黙に滲ませる
- **文体は丁寧体（です・ます）、一人称は「私」**
...
- **権威は表に出さない**: 自己ポジショニング明示は禁止。専門性は地の文で名乗らず、使う言葉の精度・機序の指摘・論点の接続で暗黙に滲ませる(詳細は `japanese_writing_style.md` 第 7 節)

引用元の内容に応じて切り口を変える(同方向の補強 / データ裏付け / 同方向の展開 / 実践示唆 / トレンド位置づけ)。**「別角度の問い直し」は反論方向に流れやすいため除外**。

### Step 6.5: 同意スタンスチェック (新規・必須)

各コメント案に対して、引用元への同意スタンスが保たれているかを `agreement_check.py` で機械的に検証する:

```bash
python3 .claude/skills/_x-shared/scripts/agreement_check.py --text "<コメント本文>"
```

判定:
- **BLOCK** → 否定・反論・相対化・敵視メタファー等が検出された。**そのコメントを再生成必須(最大 2 回)**。3 回目でも BLOCK なら **その候補ツイートを差し替え**(別の引用元に対してコメント生成)
